...
var showRoute = function(req, res){
    ...
    Activity.findById(req.params.id).populate({path: 'comments', options: { sort: { 'createdAt': -1 } } }).exec(function(err, foundActivity){ //inside the returned object "foundActivity" it will now contain actual comments and not just reference ids
